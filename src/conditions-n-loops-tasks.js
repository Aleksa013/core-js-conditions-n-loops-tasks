...
function sortByAsc(arr) {
  ...
  function quickFix(one, last) {
    const middle = sortArr[Math.floor((one + last) / 2)];
    let start = one;
    let end = last;
    let box = 0;
    while (start <= end) {
      while (sortArr[start] < middle) {
        start += 1;
      }
      while (sortArr[end] > middle) {
        end -= 1;
      }
    ...
    if (one < start - 1) {
      quickFix(one, start - 1);
    }
  }
  quickFix(0, sortArr.length - 1);
  return sortArr;
}
